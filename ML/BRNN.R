...
# Set seed number
seednumber <- c(2220); set.seed(seednumber)
...
BRNNmodel <- train(logEC ~ ., data = train, method = "brnn", trControl = train.control)
print(BRNNmodel)



# Model performance
predictions <- BRNNmodel %>% predict(test)
predictions_train <- BRNNmodel %>% predict(train)
...
DescImportance <- data.frame(Descriptor = row.names(varImp(BRNNmodel, scale=TRUE)$importance),
                             Value = varImp(BRNNmodel, scale=TRUE)$importance)
...
ExcelFile <- createWorkbook("QNAR_ANN")
...
writeData(ExcelFile, sheet = 5, capture.output(BRNNmodel))
saveWorkbook(ExcelFile, paste(dirname(QSARFile),"/BRNN_",tools::file_path_sans_ext(basename(QSARFile)),".xlsx", sep = ""), overwrite = TRUE)


# Save model for later use
BRNNDmix <- BRNNmodel
save(BRNNDmix, file = paste(dirname(QSARFile),"/BRNN",tools::file_path_sans_ext(basename(QSARFile)),".Rdata", sep = ""))
