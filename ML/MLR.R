...
# Set seed number
seednumber <- c(2245); set.seed(seednumber)
...
MLRmodel <- train(logEC ~ ., data = train, method = "lm", trControl = train.control)
print(MLRmodel)
...
DescImportance <- data.frame(Descriptor = row.names(varImp(MLRmodel, scale=TRUE)$importance),
                             Value = varImp(MLRmodel, scale=TRUE)$importance)
...
ExcelFile <- createWorkbook("QNAR_MLR")
...
writeData(ExcelFile, sheet = 5, capture.output(MLRmodel))
saveWorkbook(ExcelFile, paste(dirname(QSARFile),"/MLR_",tools::file_path_sans_ext(basename(QSARFile)),".xlsx", sep = ""), overwrite = TRUE)
