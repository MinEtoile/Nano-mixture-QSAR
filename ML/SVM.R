...
# Set seed number
seednumber <- c(4270); set.seed(seednumber)
  ...
  SVMmodel <- train(logEC ~ ., data = train, method = "Kernel", trControl = train.control)
  print(SVMmodel)
...
DescImportance <- data.frame(Descriptor = row.names(varImp(SVMmodel, scale=TRUE)$importance),
                             Value = varImp(SVMmodel, scale=TRUE)$importance)
...
ExcelFile <- createWorkbook("QNAR_SVM")
...
writeData(ExcelFile, sheet = 5, capture.output(SVMmodel))
saveWorkbook(ExcelFile, paste(dirname(QSARFile),"/SVM_",tools::file_path_sans_ext(basename(QSARFile)),".xlsx", sep = ""), overwrite = TRUE)


# Save model for later use
SVMDmix <- SVMmodel
save(SVMDmix, file = paste(dirname(QSARFile),"/SVM",tools::file_path_sans_ext(basename(QSARFile)),".Rdata", sep = ""))
